...
def draw_3d_scatters_5(df_list: List[Union[DataFrame]], random_rate=1.0):
    # 将点云concat
    df = pd.concat(df_list)
    df = df.dropna()
    # 归一化
    df[['lat', 'lon', 'geoaltitude', 'velocity', 'heading']] = df[
        ['lat', 'lon', 'geoaltitude', 'velocity', 'heading']].apply(lambda x: (x - np.min(x)) / (np.max(x) - np.min(x)))
    # 在控制台打印航向和航速的基本信息
    print("航速：")
    ...
    zs = temp_nd[:, 2]
    m = 'o'
    ax.scatter(xs, ys, zs, marker=m, s=5, c='#7f7f7f', alpha=1.0)
    # 绘制有分类的点云
    for cluster_index in topk_cluster:
        ...
        zs = temp_nd[:, 2]
        m = 'o'
        ax.scatter(xs, ys, zs, marker=m, s=2)
    ax.legend(['类别top'] + [f"第{str(i + 1)}类" for i, c in enumerate(topk_cluster)])
    ax.set(xlabel='lat')
    ax.set(ylabel='lon')
    ax.set(zlabel='geoaltitude')
    ax.set_title("航迹点云聚类（经纬高航向航速）")
    plt.show()


# 不加航向航速
def draw_3d_scatters_3(df_list: List[Union[DataFrame]], random_rate=1.0):
    # 将点云concat
    df = pd.concat(df_list)
    df = df.dropna()
    # 归一化
    ...
    for cluster_index in topk_cluster:
        ...
        zs = temp_nd[:, 2]
        m = 'o'
        ax.scatter(xs, ys, zs, marker=m, s=5)
    ax.legend(['类别top'] + [f"第{str(i + 1)}类" for i, c in enumerate(topk_cluster)])
    ax.set(xlabel='lat')
    ax.set(ylabel='lon')
    ax.set(zlabel='geoaltitude')
    ax.set_title("航迹点云聚类(经纬高)")
    plt.show()


if __name__ == '__main__':
    df_list = read_csvs('csv', 'D:\Repo\dbscan_absd\爬取数据\M-C5M运输_AE056D')
    draw_3d_scatters_5(df_list, random_rate=0.012)  # 加航向航速聚类
    draw_3d_scatters_3(df_list, random_rate=0.012)  # 不加航向航速聚类
